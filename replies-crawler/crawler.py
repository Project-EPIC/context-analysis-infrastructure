...
RESULTS_DIR = os.path.join(BASE_DIR, 'replies')
GEORESULTS_DIR = os.path.join(BASE_DIR, 'georeplies')
logging.basicConfig(
...
def pull_user_replies(input_line, is_geo_enabled=True):
    user, event, start_date, end_date = _unpack(input_line)
    user = slugify(user)
    logging.info('Pulling replies to %s...' % user)
    cursor = Cursor(api.search, q='@%s filter:replies' % user, count=100)
    ...
    logging.info('Pulling replies to %s...Done' % user)
    return classified_filename, classified_geofilename


def generate_pull_bucket(bucket):
    ...
    def func(input_line, is_geo_enabled):
        ...
        for status in ALL_STATUS:
            ...
            if _blob_exists(filename, b):
                return filename
        filename, geofilename = pull_user_replies(input_line, is_geo_enabled)
        _upload_file_gcloud(filename, b)
        if geofilename:
            _upload_file_gcloud(geofilename, b)
        return filename
...
if __name__ == '__main__':
    ...
    pull_function = pull_user_replies

    if args.bucket:
        pull_function = generate_pull_bucket(args.bucket)
    ...
    if args.filename:
        ...
        consumer = KafkaConsumer(topic, group_id='reply_puller',
